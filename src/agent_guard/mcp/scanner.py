...
SUSPICIOUS_PATTERNS = [
    (r"ignore\s+(?:all\s+)?(?:previous|above|prior)\s+instructions",
     ThreatType.INJECTION, ThreatSeverity.CRITICAL,
    ...
    (r"<\s*(?:script|img|iframe|object|embed|svg)\b",
     ThreatType.HIDDEN_INSTRUCTIONS, ThreatSeverity.HIGH,
    ...
    (r"(?:system|admin|root|sudo)\s*(?:access|privilege|permission|mode)",
     ThreatType.PRIVILEGE_ESCALATION, ThreatSeverity.HIGH,
     "Tool description references privilege escalation"),
    (r"(?:exec|eval|compile|subprocess|os\.system|popen)\s*\(",
     ThreatType.TOOL_POISONING, ThreatSeverity.CRITICAL,
    ...
    (r"(?:password|secret|token|api[_-]?key|credential)s?\s*[:=]",
     ThreatType.TOOL_POISONING, ThreatSeverity.HIGH,
     "Hardcoded secret or credential in tool definition"),
    (r"(?:curl|wget|fetch)\s+https?://",
     ThreatType.TOOL_POISONING, ThreatSeverity.MEDIUM,
     "External URL fetch in tool definition"),
    (r"(?:base64|atob|btoa)\s*(?:\(|\.decode)",
     ThreatType.HIDDEN_INSTRUCTIONS, ThreatSeverity.MEDIUM,
     "Base64 encoding/decoding — may hide malicious payloads"),
]

KNOWN_TOOL_NAMES: set[str] = {
    "web_search", "file_read", "file_write", "code_exec", "shell_exec",
    "database_query", "http_request", "send_email", "browser",
    "calculator", "python_repl", "retrieval", "dalle",
}
        ...
        for known in self._known_tools:
            ...
            if 0 < dist <= 2:
                findings.append(ThreatFinding(
                    ...
                    tool_name=name,
                    description=f"Tool name '{name}' is suspiciously similar to known tool '{known}' "
                                f"(edit distance: {dist})",
                    evidence=f"{name} ≈ {known}",
                    recommendation="Verify this is the intended tool and not a malicious impersonator",
                ))
        return findings
    ...
    def _check_schema(self, tool_name: str, schema: dict[str, Any]) -> list[ThreatFinding]:
        ...
        if len(props) > 20:
            findings.append(ThreatFinding(
                ...
                severity=ThreatSeverity.MEDIUM,
                tool_name=tool_name,
                description=f"Excessive parameter count ({len(props)}) — may indicate schema abuse",
...
def _recommendation_for(threat_type: ThreatType) -> str:
    recs = {
        ...
        ThreatType.TYPOSQUATTING: "Verify this is the intended tool and not a malicious impersonator",
